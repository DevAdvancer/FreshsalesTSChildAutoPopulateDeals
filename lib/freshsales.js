...
const getDealFieldChoices = async (fieldName) => {
  const client = getClient();
  const response = await client.get('/settings/deals/fields');
  const field = response.data.fields?.find(item => item.name === fieldName);
  if (!field) {
    throw new Error(`Deal field ${fieldName} was not found in Freshsales settings.`);
  }
  return new Set((field.choices || []).map(choice => String(choice.value)));
};
